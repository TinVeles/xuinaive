...
xui_apply_warp_template() {
  ...
  local db tags_json domains_json current key snippet_file updated
  db="$(xui_db_path)"
  ...
  key="$(sqlite3 -readonly "$db" "SELECT key FROM settings WHERE key IN ('xrayTemplateConfig','xrayConfig','xraySetting') LIMIT 1;" || true)"
  ...
  if ! jq -e . >/dev/null 2>&1 <<<"$current"; then
    upm_log_warn "x-ui setting $key is not valid JSON. Saved WARP routing snippet only: $snippet_file"
    return 0
  fi
  ...
  upm_log_ok "x-ui WARP outbound/routing saved in settings.$key"
  upm_log_ok "WARP routing snippet saved: $snippet_file"
}

xui_remove_warp_template() {
  local db key current updated snippet_file
  db="$(xui_db_path)"
  snippet_file="/etc/x-ui/warp-generated-routing.json"
  [[ -f "$db" ]] || return 0
  key="$(sqlite3 -readonly "$db" "SELECT key FROM settings WHERE key IN ('xrayTemplateConfig','xrayConfig','xraySetting') LIMIT 1;" || true)"
  ...
  [[ -n "$current" ]] || return 0
  if ! jq -e . >/dev/null 2>&1 <<<"$current"; then
    upm_log_warn "x-ui setting $key is not valid JSON; WARP template cleanup skipped"
    return 0
  fi
  ...
  upm_log_ok "x-ui WARP outbound/routing removed from settings.$key"
}
