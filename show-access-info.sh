...
if [[ "$nh_backend_kind" == "rixxx-naive-mieru" ]]; then
  nh_panel_url="$(first_nonempty "$(config_value RIXXX_PANEL_URL)" "$(config_value NH_PANEL_URL)" "$(rixxx_json_value panelUrl)" "http://127.0.0.1:3000/")"
  nh_panel_login="$(first_nonempty "$(config_value RIXXX_PANEL_LOGIN)" "$(config_value NH_PANEL_LOGIN)" "$(rixxx_json_value panelLogin)")"
  nh_panel_password="$(first_nonempty "$(config_value RIXXX_PANEL_PASSWORD)" "$(config_value NH_PANEL_PASSWORD)" "$(rixxx_json_value panelPassword)")"
  config_set_file "$CONFIG_FILE" NH_PANEL_URL "$nh_panel_url"
  ...
  config_set_file "$CONFIG_FILE" RIXXX_PANEL_URL "$nh_panel_url"
  [[ -n "$nh_panel_login" ]] && config_set_file "$CONFIG_FILE" RIXXX_PANEL_LOGIN "$nh_panel_login"
  [[ -n "$nh_panel_password" ]] && config_set_file "$CONFIG_FILE" RIXXX_PANEL_PASSWORD "$nh_panel_password"
  ...
  config_set_file "/etc/rixxx-panel/access-info.env" RIXXX_PANEL_URL "$nh_panel_url"
  [[ -n "$nh_panel_login" ]] && config_set_file "/etc/rixxx-panel/access-info.env" RIXXX_PANEL_LOGIN "$nh_panel_login"
  [[ -n "$nh_panel_password" ]] && config_set_file "/etc/rixxx-panel/access-info.env" RIXXX_PANEL_PASSWORD "$nh_panel_password"
else
