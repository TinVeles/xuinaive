...
async function renderQuickLinks(status) {
  ...
  // Combined subscriptions generated by unified-proxy-manager
  try {
    const subRes = await fetch('/api/subscriptions');
    if (subRes.ok) {
      const subs = await subRes.json();
      if (subs.available) {
        if (subs.v2rayn?.stable) {
          hasAny = true;
          listEl.innerHTML += `
            <div class="quick-link-item">
              <span class="ql-type sub-tag">Sub</span>
              <span class="ql-user">v2rayN stable</span>
              <span class="ql-url">${escapeHtml(subs.v2rayn.stable)}</span>
              <button class="quick-link-copy" data-copy="${escapeHtml(subs.v2rayn.stable)}">Копировать</button>
            </div>`;
        }
        if (subs.v2rayn?.txt) {
          hasAny = true;
        ...
        if (subs.combined?.txt) {
          ...
          listEl.innerHTML += `
            <div class="quick-link-item">
              <span class="ql-type sub-tag">Sub</span>
              <span class="ql-user">combined</span>
              <span class="ql-url">${escapeHtml(subs.combined.txt)}</span>
              <button class="quick-link-copy" data-copy="${escapeHtml(subs.combined.txt)}">Копировать</button>
            </div>`;
        }
        if (subs.singBox) {
          hasAny = true;
          listEl.innerHTML += `
            <div class="quick-link-item">
              <span class="ql-type sub-tag">Sub</span>
              <span class="ql-user">sing-box</span>
              <span class="ql-url">${escapeHtml(subs.singBox)}</span>
              <button class="quick-link-copy" data-copy="${escapeHtml(subs.singBox)}">Копировать</button>
            </div>`;
        }
        (subs.users || []).slice(0, 15).forEach(sub => {
          if (!sub.txt) return;
          hasAny = true;
          listEl.innerHTML += `
            <div class="quick-link-item">
              <span class="ql-type sub-tag">Sub</span>
              <span class="ql-user">${escapeHtml(sub.name)}</span>
              <span class="ql-url">${escapeHtml(sub.txt)}</span>
              <button class="quick-link-copy" data-copy="${escapeHtml(sub.txt)}">Копировать</button>
            </div>`;
        });
      }
    }
