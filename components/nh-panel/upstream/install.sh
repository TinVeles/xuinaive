...
if [[ -n "${LOCAL_PANEL_SOURCE}" && -d "${LOCAL_PANEL_SOURCE}/panel" ]]; then
  log_info "Использую локальную копию панели: ${LOCAL_PANEL_SOURCE}"
  rm -rf "${PANEL_DIR}"
  mkdir -p "${PANEL_DIR}"
  cp -a "${LOCAL_PANEL_SOURCE}/." "${PANEL_DIR}/"
...
Environment=WARP_PROXY_HOST=${WARP_PROXY_HOST}
Environment=WARP_PROXY_PORT=${WARP_PROXY_PORT}
Environment=WARP_OUTBOUND_TAG=${WARP_OUTBOUND_TAG}
Environment="WARP_AI_DOMAINS=${WARP_AI_DOMAINS}"
StandardOutput=journal
StandardError=journal
