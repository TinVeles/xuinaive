...
- RIXXX Panel listens on `3000` and is exposed through nginx `8081` by default.

Use `../install-unified.sh --mode all --yes` for the explicit real installer.

`components/rixxx-panel/install-unified-backend.sh` is the NaiveProxy backend used by the all-in-one flow. It installs RIXXX Panel + NaiveProxy + Hysteria2 without taking public `443/tcp` away from nginx.
...
- RIXXX panel for NaiveProxy + Hysteria2.
